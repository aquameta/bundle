...
-- track one row
do $$ begin
    perform bundle.track_untracked_row('io.pgbundle.unittest', meta.row_id('pt', 'periodic_table', 'AtomicNumber', '7'));
...
-- track again
select throws_ok(
    $$ select bundle.track_untracked_row('io.pgbundle.unittest', meta.row_id('pt', 'periodic_table', 'AtomicNumber', '7')); $$,
...
do $$ begin
    perform bundle.track_untracked_row('io.pgbundle.unittest', meta.row_id('unittest', 'not_a_table', 'a', '1'));
end $$ language plpgsql;
...
do $$ begin
    perform bundle.untrack_tracked_row('io.pgbundle.unittest', meta.row_id('pt', 'periodic_table', 'AtomicNumber', '7'));
end $$ language plpgsql;
...
do $$ begin
    perform bundle.untrack_tracked_row('io.pgbundle.unittest', meta.row_id('unittest', 'not_a_table', 'a', '1'));
end $$ language plpgsql;
...
select throws_ok(
    $$ select bundle._untrack_tracked_row(bundle.repository_id('io.pgbundle.unittest'), meta.row_id('pt', 'periodic_table', 'AtomicNumber', '3'::text)) $$,
    format(
