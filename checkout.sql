...
begin
    ...
    for r in select delta.commit_rows(_commit_id) loop
...
begin
    -- commit exists
    if not delta.commit_exists(_commit_id) then
        raise exception 'Commit with id % does not exist.', _commit_id;
    end if;

    -- repo has no uncommitted changes
    if delta.repository_has_uncommited_changes(_repository_id) then
        raise exception 'Repository % has uncommited changes, checkout() cannot be performed.', delta.repository_name(_repository_id);
    end if;

    -- propagate vars
    select r.id, r.name, c.head_commit_id, c.checkout_commit_id, c.message from delta.repository r join delta.commit c on c.repository_id = r.id
    ...
    /*
    for commit_row in select * from delta.commit_rows(_commit_id) r join delta.commit_fields(_commit_id) f on f.field_id)::meta.row_id = r.row_id group by r.row_id
    loop
        select delta._checkout_row(commit_row);
    end loop;
    */
...
create function _checkout_row( commit_row record ) returns void as $$
declare
    insert_stmt text;
begin
    raise notice '_checkout_row( % )', commit_row;
    return;
end
$$ language plpgsql;
